...
private:
    ...
    Eigen::VectorXd wheel_velocity;  // 車輪の速度ベクトル定義  
...
public:
    Hopping_wheel_Controller(void){
        ...
        wheel_velocity.resize(1);  // wheel_velocityの要素数の変更
    ...
    virtual void loop(const ros::TimerEvent& e) override {
        ...
        if(phase == 0){
            if(!initializeFlag){
                ...
                joint_Interpolator.appendSample(current_time, q_init.head<3>());  // 現在の関節角度
            ...
            // qref.head<2>() = joint_Interpolator.interpolate(current_time);  // 先頭2要素に代入
        ...
        // 車輪走行して目標速度まで補間
            ...
            */
            ...
            if(initializeFlag == false){
                ROS_INFO("Start controller!");
                joint_Interpolator.clear();
                joint_Interpolator.appendSample(current_time, 0);
                // ROS_INFO("wheel = %.2f[rad]", q_initialize[WHEEL]);
                q_initialize[WHEEL] = wheel_movingDistance/wheel_D;
                // ROS_INFO("wheel delta = %.2f[rad]", q_initialize[WHEEL]);
                joint_Interpolator.appendSample(current_time+wheel_accelerateTime, wheel_maxVelocity);
                q_initialize[WHEEL] = 2*wheel_movingDistance/wheel_D;
                joint_Interpolator.appendSample(current_time+wheel_movingTime, 0);
                joint_Interpolator.update();
                initializeFlag = true;
            }
            ...
            if(current_time > joint_Interpolator.domainUpper()-0.5){ // 車輪移動時間[s] - 0.5[s]後であれば
                initializeFlag = false;
                phase = 2;
            }
        }
